...
abar = pygame.Surface((60,15)) 
bar = abar.convert()
bar.fill((255,100,100))
circ_sur = pygame.Surface((20,20)) 
circ = pygame.draw.circle(circ_sur,(255,255,255),(20/2,20/2),20/2)
...
class GameState:
	...
	def __init__(self):
		self.bar_x, self.bar_y = 150.,300.
		self.circle_x, self.circle_y = 160.,40.
		self.bar_move = 0.
		self.speed_x, self.speed_y, self.speed_circ = 15., 18., 15. 
		self.bar_score = 0
		self.hi_score = 0
	...
	def gameframe(self,input_vector):
		...
		frame = pygame.draw.rect(screen,(255,255,255),Rect((5,25),(340,290)),2) 
		...
		if self.circle_x >= 345.:
			self.circle_x = 345.
			self.speed_x = - self.speed_x 
		if self.circle_y >= self.bar_y - 15.:
			if self.circle_x >= self.bar_x - 5 and self.circle_x <= self.bar_x + 75:
